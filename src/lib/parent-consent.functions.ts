...
import { sendConsentFormReminders } from "@/lib/consent-reminders";
...
/** Admin-only: email consent form link to parents of unsigned teammates. */
export const sendConsentFormReminderEmails = createServerFn({ method: "POST" })
  .inputValidator((data: unknown) => adminSchema.parse(data))
  .handler(async ({ data }) => {
    await assertAdmin(data.accessToken);
    return sendConsentFormReminders();
  });
